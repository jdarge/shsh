...
char *read_line(char *buffer_input, int position_input, ENV *env, History *history, int flag) {
    ...
    while (1) {
        ...
        if (current_user_input == '\t') {   // TODO! this need to be put into it's own function or simplified...
        ...
        if (current_user_input == EOF || current_user_input == '\n') {
            ...
            printf("\n");
            return buffer;
        } else if (current_user_input == ESCAPE) {   // TODO! add ctrl + L/R Arrow handler to jump around whitespace
        ...
        else if (!ctrl_c_pressed) {   // Ran through all checks
